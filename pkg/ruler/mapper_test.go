...
func Test_FSLoader_ParseCache(t *testing.T) {
	...
	t.Run("mutating a cache-hit result doesn't corrupt a later cache hit", func(t *testing.T) {
		// Regression test for a narrower case than the one above: a hit must
		// also return its own copy, not the pointer promoted between
		// generations. Returning entry.parsed directly on a hit would pass
		// every other test in this file, since NotSame above only compares a
		// miss result against a hit result, never a hit against a hit.
		setupRuleSets()
		fs := afero.NewMemMapFs()
		m := &mapper{Path: "/rules", FS: fs, logger: l}
		ruleConfigs := map[string][]rulefmt.RuleGroup{
		...
		firstHit, errs := loader.Load(files[0], false, model.LegacyValidation)
		require.Empty(t, errs)
		require.Equal(t, 1.0, testutil.ToFloat64(hits))

		slices.Sort(firstHit.Groups[0].SourceTenants)
		require.Equal(t, []string{"tenant-a", "tenant-b"}, firstHit.Groups[0].SourceTenants)

		secondHit, errs := loader.Load(files[0], false, model.LegacyValidation)
		require.Empty(t, errs)
		require.Equal(t, 2.0, testutil.ToFloat64(hits))
		require.Equal(t, []string{"tenant-b", "tenant-a"}, secondHit.Groups[0].SourceTenants,
			"the cached entry must be unaffected by mutating a previously returned hit result")
		require.NotSame(t, firstHit, secondHit)
	})
