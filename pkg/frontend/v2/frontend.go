...
type Config struct {
	...
	EnqueueRetryEnabled bool              `yaml:"scheduler_enqueue_retry_enabled" category:"experimental"`
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.BoolVar(&cfg.EnqueueRetryEnabled, "query-frontend.scheduler-enqueue-retry-enabled", false, "If enabled, when a query-scheduler rejects a query because its per-tenant queue is full, retry enqueuing the query on another query-scheduler which has not been tried yet.")
